...
- Entries: **52,045**
- Size on disk: **6.5 MB** JSON
- In-memory format: JSON loaded into a Python dict (O(1) lookup, ~0.0004 ms). marisa-trie is available as a memory optimization but unnecessary at this size (the dict costs ~21 MB RSS)
...
### 4.3 Burst, Allocator, and the Breaking Point

Three more INT8 runs on the same 4-core config characterize the operating envelope:
...
- **Breaking point.** The pessimistic per-keystroke run offers ~1,750 RPS; the 4-core box delivers ~950 RPS and latency climbs to p50 200 ms (server-side compute is still microseconds for dict hits and ~14 ms p99 for the model, so the 200 ms is queueing at a saturated box, not slow inference). So **~950 RPS is the saturation ceiling, not deployable capacity**. The burst run held p95 48 ms at 788 RPS, so ~800 RPS is a reasonable planning number, but it comes from a single 3-minute burst with large p99/p99.9 tails; a production sizing should confirm it with steady-state runs at 700/750/800 RPS plus headroom and an SLO (listed in section 7.3). The ceiling is set by aggregate HTTP throughput, not the model path.
...
- The server ran on 4 vCPUs; the load generator ran separately so it never competes for the server's cores.
...
- **Steady-state capacity + real-instance testing.** Confirm the ~800 RPS/box planning number with sustained 700/750/800 RPS runs, an SLO and headroom, and a real n2-standard-4 behind TLS + a load balancer (the current numbers are a same-box CPU-capacity approximation).
